...
import io.softa.starter.permission.annotation.RequirePermissions;
import lombok.RequiredArgsConstructor;

/**
 * Fail-loud at startup: resolve every {@code @RequirePermission} declaration in
...
public class RequirePermissionStartupValidator implements SmartInitializingSingleton {
    ...
    public void afterSingletonsInstantiated() {
        for (Object bean : applicationContext.getBeansWithAnnotation(Controller.class).values()) {
            ...
            for (Method method : target.getMethods()) {
                if (AnnotatedElementUtils.hasAnnotation(method, RequirePermission.class)
                        || AnnotatedElementUtils.hasAnnotation(method, RequirePermissions.class)) {
                    // Throws IllegalStateException with the concrete fix on any mismatch.
                    RequirePermissionAspect.resolve(method);
                }
            }
        }
    }
