...
abstract class AbstractDdlOrchestratorTest {
    ...
    // ---- scenario C2: narrowing MODIFY executes (this lane plans without facts) ----

    @Test
    void narrowingModify_executes_planningHasNoFacts() {
        // The metadata-only lane is only reachable when physical introspection failed, so
        // it cannot judge widths — the declared shape executes as-is. A boot with facts
        // takes the convergence lane instead (see AbstractDdlConvergeTest), where the same
        // narrowing is deliberate policy.
        ...
        Integer length = jdbcTemplate.queryForObject(
                "SELECT CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS "
                        + "WHERE LOWER(TABLE_NAME) = LOWER(?) AND LOWER(COLUMN_NAME) = LOWER(?)",
                Integer.class, "customer", "email");
        assertEquals(64, length, "no facts — the declared MODIFY executes as-is");
    }
    ...
    void modifyColumn_onMissingColumn_failsFast() {
        // This lane plans purely from the metadata diff (it is only reachable when
        // introspection failed), so genuine drift behind a planned MODIFY stays a hard
        // boot failure (rows unwritten, retried). A boot with facts heals the same state
        // on the convergence lane instead (see AbstractDdlConvergeTest).
        jdbcTemplate.execute("CREATE TABLE customer (id BIGINT NOT NULL PRIMARY KEY)");
        ...
        assertThrows(org.springframework.dao.DataAccessException.class,
                () -> orchestrator.apply(diff, codeModels(diff, customer()), fields));
    }
