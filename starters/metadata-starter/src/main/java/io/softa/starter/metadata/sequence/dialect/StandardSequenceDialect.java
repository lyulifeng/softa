 ...
 *
 * <p><b>Oracle caveat</b>: {@code IS NOT DISTINCT FROM} is ISO SQL but Oracle does not
 * support it (SQL Server does from 2022). Neither engine can run metadata-starter today —
 * {@code DdlDialectFactory} rejects both, so {@code sys_sequence} cannot even bootstrap there.
 * When Oracle support becomes real, split it into its own dialect class, as
 * {@code SequenceDialectFactory}'s javadoc already prescribes.
 */
@Component
