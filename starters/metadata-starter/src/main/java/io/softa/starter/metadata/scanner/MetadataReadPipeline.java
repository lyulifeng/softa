...
public class MetadataReadPipeline {
    ...
    /**
     * Parse the given models / option-set enums into the from-code state, then append the
     * search indexes derived from {@code @Model(searchName = ...)} — see
     * {@link SearchIndexSynthesizer}.
     *
     * <p>The derivation belongs HERE, not in {@link AnnotationParser} and not in the scanner,
     * because this method is the only seam both {@link MetadataAnnotationScanner} (write lane)
     * and {@link MetadataAnnotationChecker} (read-only production lane) go through. Deriving on
     * one side only would make the two lanes' from-code fingerprints disagree, and the checker
     * would then report permanent drift on every boot. Same reasoning, same layer as
     * {@code ReferenceColumnResolver.stampSysFields}.
     *
     * <p>Because the rows land in the from-code set, they are declared indexes in every
     * downstream sense: they enter the diff, get written to {@code sys_model_index}, are created
     * by convergence, and — critically — join {@code DdlOrchestrator}'s declared-name set, so
     * the undeclared-index DROP never reaps them.
     */
    public AnnotationScanResult parse(Collection<Class<?>> modelClasses,
        ...
        List<SysModelIndex> derived = SearchIndexSynthesizer.derive(
                ...
                SearchIndexSpecs.declared(parsed.modelIndexes()));
        if (derived.isEmpty()) {
            return parsed;
        }
