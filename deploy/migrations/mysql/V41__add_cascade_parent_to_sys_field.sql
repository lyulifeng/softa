...
ALTER TABLE sys_field
    ADD COLUMN cascade_parent TINYINT(1) DEFAULT 0 COMMENT 'Cascade Parent;The MANY_TO_ONE a dependent import-template dropdown narrows by'
    AFTER auto_sequence;
UPDATE sys_field SET cascade_parent = 0 WHERE cascade_parent IS NULL;

ALTER TABLE design_field
    ADD COLUMN cascade_parent TINYINT(1) DEFAULT 0 COMMENT 'Cascade Parent;The MANY_TO_ONE a dependent import-template dropdown narrows by'
    AFTER auto_sequence;
UPDATE design_field SET cascade_parent = 0 WHERE cascade_parent IS NULL;
