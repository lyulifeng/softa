...
public class Filters implements Serializable {
    ...
    public static Filters of(String filterString) {
        if (StringUtils.isBlank(filterString)) {
            ...
            List<Object> filterList = JsonUtils.stringToObject(filterString, new TypeReference<>() {});
    ...
    private static Filters listToFilters(List<?> filterList) {
        ...
        if (operator != null) {
            ...
            if (filterList.size() == 1) {
                ...
                return listToFilters(firstElement);
            } else {
    ...
    private static Filters parseList(List<?> filterList) {
        ...
        for (Object node : filterList) {
            if (node instanceof List<?> listNode) {
                Filters child = listToFilters(listNode);
                // Ignore EMPTY node
                if (!Filters.isEmpty(child)) {
                    filters.getChildren().add(child);
                }
            } else if (node instanceof String strNode) {
                parseLogicOperator(filters, strNode, filterList);
            } else {
    ...
    public boolean equals(Object filters) {
        if (filters instanceof Filters filtersObj) {
            return Objects.equals(type, filtersObj.getType())
                    && Objects.equals(logicOperator, filtersObj.getLogicOperator())
                    && Objects.equals(children, filtersObj.getChildren())
                    && Objects.equals(filterUnit, filtersObj.getFilterUnit());
        }
        return false;
    }
