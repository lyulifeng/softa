...
public abstract class DateUtils {
    ...
    public static Object stringToDateObject(String value, Class<?> dateType) {
        ...
        } else if (dateType.equals(LocalTime.class)) {
            return LocalTime.parse(value, TimeConstant.TIME_FORMATTER);
        }
        return value;
    }
    ...
    public static String dateToString(Object date, DateTimeFormatter formatter) {
        return switch (date) {
            case null -> null;
            case LocalDate localDate -> formatter.format(localDate);
            case Date ignored -> formatter.format(dateToLocalDate(date));
    ...
    public static LocalDate dateToLocalDate(Object date) {
        return switch (date) {
            ...
            case Date javaDate -> javaDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            case String stringDate -> LocalDate.parse(stringDate, TimeConstant.DATE_FORMATTER);
    ...
    public static LocalTime stringToLocalTime(String timeStr) {
        if (StringUtils.isBlank(timeStr)) {
            ...
            return LocalTime.parse(timeStr, TimeConstant.SIMPLE_TIME_FORMATTER);
        } else if (timeStr.length() == 8) {
            return LocalTime.parse(timeStr, TimeConstant.TIME_FORMATTER);
        } else {
            log.error("Invalid time string: {}", timeStr);
            return null;
        }
    }
